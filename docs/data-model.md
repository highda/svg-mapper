...
`customCss` is an advanced, portable view override. The renderer prefixes every selector with its unique instance target, renames keyframes, and mounts only the active view's stylesheet. Inspector-authored SVG presentation attributes remain the baseline; normal CSS declarations override them, while inline runtime state such as cursor and overlay placement may require `!important`. Imports, `url()` resources, global resource rules, nesting, and unknown at-rules are rejected. Supported grouping rules are `@media`, `@supports`, `@container`, and `@layer`; `@keyframes` and `@-webkit-keyframes` are renamed per instance.
